...
import com.wxf.service.ILoginService;
  ...
  public int saveOrUpdateAdminInf(AdminInfoBean adminB) {
    ...
    if(adminB != null ) {
      ...
      if(adminB.getAdminid() == null) {
        // 新增sql语句
        String strSql = "insert into admininfo(adminname,adminpsw,adminaddress,admintel,regdate,adminimg) values(?,?,?,?,to_date(?,'yyyy-mm-dd hh24:mi:ss'),?)";        // 调用db层进行数据库操作
        i = MyUtils.getInstance(DBProvider.class).upade(strSql, adminB.getAdminname(),adminB.getAdminpsw(),adminB.getAdminaddress(),adminB.getAdmintel(),MyUtils.convertDate2String(adminB.getRegdate(), "yyyy-MM-dd HH:mm:ss"),adminB.getAdminimg());
      }else {
        // 修改操作
      }
    }
